...
class JadwalSemhasModel extends Model
{
    ...
    {
        $tahun = $tahun ?? date('Y');
        return $this->select('mahasiswa.nama as nama_mhs, mahasiswa.nim as nim,  mahasiswa.prodi as prodi, u3.id as id_mhs, u1.nama as penguji1, simta_acc_judul.judul_acc as judul, simta_rilis_jadwal_semhas.*, simta_pengajuan_seminarhasil.status_laporan as status_laporan, simta_pengajuan_seminarhasil.id_seminarhasil as id_seminarhasil, simta_pengajuan_seminarhasil.revisi_laporan as revisi_laporan')
    ...
    public function getBeritaAcara($id) 
    {
        return $this->select('mahasiswa.nama as nama_mhs, mahasiswa.nim as nim,  mahasiswa.prodi as prodi, u3.id as id_mhs, u1.nama as penguji1, s1.nip as nip_penguji1, simta_acc_judul.judul_acc as judul, dosen.nama, simta_pengajuan_seminarhasil.*, simta_rilis_jadwal_semhas.*,
                    GROUP_CONCAT(CASE WHEN simta_revisi_semhas.id_penguji = u1.id THEN simta_revisi_semhas.catatan_revisi END SEPARATOR " | ") as catatan_revisi_penguji1')
            ->join('users as u1', 'simta_rilis_jadwal_semhas.id_penguji1=u1.id')
            // ->join('users as u2', 'simta_rilis_jadwal_semhas.id_penguji2=u2.id')
            ->join('simta_pengajuan_seminarhasil', 'simta_rilis_jadwal_semhas.id_pengajuansemhas=simta_pengajuan_seminarhasil.id_seminarhasil')
            ->join('users as u3', 'simta_pengajuan_seminarhasil.id_mhs=u3.id')
            ...
            ->join('simta_revisi_semhas', 'simta_rilis_jadwal_semhas.id_rilis_jadwal_semhas=simta_revisi_semhas.id_rilis_jadwal_semhas', 'left') // Join with revisions table
            ->groupBy('simta_rilis_jadwal_semhas.id_rilis_jadwal_semhas')
            ->find($id);
    }
